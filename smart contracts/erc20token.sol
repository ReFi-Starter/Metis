...
contract Token is Context, ERC20, Ownable {
    ...
    constructor(string memory name,string memory symbol,address payable __mktgWallet,uint256 _totalsupply,uint256 _mrktingfee,uint256 _devfee,uint256 _liqdtyfee,address payable _owneraddress ) ERC20(name, symbol) Ownable(_owneraddress) {
        _excludedFees[owner()] = true;
       _mktgWallet = payable(__mktgWallet);
        _devWallet = payable(__mktgWallet);
        _liqWallet = payable(__mktgWallet);
        _tSupply = _totalsupply * (10**18);
        buyMktgFee = _mrktingfee;
        buyDevFee = _devfee;
        buyLiqFee = _liqdtyfee;
        sellMktgFee = _mrktingfee;
        sellDevFee = _devfee;
        sellLiqFee = _liqdtyfee;
        swapEnabled = false;
    ...
    function openTrading(address _router) public onlyOwner {
        require(!tradingOpen, "Trading is already open");
        swapEnabled = true;
        maxBuyAmount = _tSupply;
        maxSellAmount = _tSupply;
        maxWalletAmount = _tSupply.mul(5).div(1000);
